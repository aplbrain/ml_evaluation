...
def main():
    ...
    parser.add_argument('--results_dir', default='.', help='The directory to store the results in.')
    ...
    cwl_file = pkg_resources.resource_filename('microns_evaluation', 'cwl/task_2a.cwl')
    images_dir = os.path.abspath(args.images_dir)

    episode_by_tag = defaultdict(dict)
    for path in os.listdir(args.csvs_dir):
        name, ext = path.split('.')
        experiment_tag, mode, id = name.split('-')
    ...
    for i, (tag, episode) in enumerate(episode_by_tag.items()):
        ...
        cmd = ['cwl-runner',
               '--no-container',
               cwl_file,
               '--trainData', episode['train'],
               '--onlineData', episode['online'],
